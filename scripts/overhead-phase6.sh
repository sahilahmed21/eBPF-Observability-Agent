...
SRV="$CARGO_TARGET_DIR/release/writev-server"
...
pkill -9 -f writev-server 2>/dev/null || true
sleep 1

PORT=$PORT "$SRV" >/tmp/ohs6.log 2>&1 &
spid=$!
sleep 0.5
...
for i in 1 2 3 4 5 6 7 8; do
  "$PR" --port "$PORT" --repeat 2 --path /fast --path '/slow?delay_ms=5' || true
...
echo "=== overhead-phase6 summary ==="
echo "load: http-probe 8 rounds × 2 repeats × 2 paths vs writev-server :$PORT"
python3 - "$PSLOG" <<'PY'
import sys
rows=[]
