...
impl PeerCache {
    pub fn insert(&mut self, tgid: u32, fd: i32, peer: PeerAddr, now: Instant) {
        if self.map.len() >= CAP {
            self.map
    ...
    pub fn get(&mut self, tgid: u32, fd: i32, now: Instant) -> Option<PeerAddr> {
